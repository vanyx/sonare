...
public interface PoliceRepository extends JpaRepository<Police, Long> {
    ...
    /**
     * Recherche tous les Police situés dans un rayon donné autour d'un point central.
     *
     ...
     * @return Liste des Police sous forme d'objets contenant l'id et la position en GeoJSON.
     */
    @Query(value = """
    ...
    /**
     * Recherche tous les Police situés à l'intérieur d'un rectangle géographique défini par deux points opposés.
     *
    ...
    /**
     * Regroupe les Police dans une zone donnée en clusters selon l'algorithme DBSCAN et retourne leur centroïde.
     *
     * @param east    Longitude minimale (bord gauche du rectangle).
     * @param south   Latitude minimale (bord inférieur du rectangle).
     * @param west    Longitude maximale (bord droit du rectangle).
    ...
    /**
     * Vérifie si un Police existe déjà à une position donnée.
     *
     * @param location Objet Point représentant la position géographique à vérifier.
     * @return True si un Police existe à cette position, sinon False.
     */
    @Query(value = "SELECT EXISTS (SELECT 1 FROM Police WHERE ST_Equals(location, ?1))", nativeQuery = true)
